...
export (Color) var light_color : Color = Color("#FFB7A6")
...
func _ready():
	...
	line_wide.default_color = light_color

	light_color.s = light_color.s + 0.23
	
	line_thin_1.width = line_width_small
	line_thin_1.default_color = light_color
	
	line_thin_2.width = line_width_small
	line_thin_2.default_color = light_color
...
func blast_enemy():
	if target.is_in_group("flotting_bomb"):
		target.blast()
